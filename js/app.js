...
const API_URL = "https://script.google.com/macros/s/AKfycbxhBq00ZbF5Pt_LuI0JlK3mopwh0mnOOtKq87FU4_Igoh_VdE-dOeYyN1VqLjEEY5Wv/exec";
...
const LAST_REMOTE_SYNC_KEY = 'lastRemoteSyncAt';
const AUTO_REMOTE_SYNC_TTL_MS = 10 * 60 * 1000;
...
const INIT_FETCH_TIMEOUT_BASE_MS = 15000;
const INIT_FETCH_TIMEOUT_PER_1000_MS = 6000;
const INIT_FETCH_TIMEOUT_MAX_MS = 90000;
const POST_TIMEOUT_MS = 20000;
const SYNC_QUEUE_PROGRESS_EVERY = 25;
const REMOTE_PAGE_SIZE = 2000;
const SYNC_BATCH_SIZE = 100;

function getAdaptiveInitTimeoutMs() {
  const estimatedCount = Math.max(Array.isArray(JUNTAS) ? JUNTAS.length : 0, 1);
  const extraBlocks = Math.ceil(estimatedCount / 1000);
  return Math.min(
    INIT_FETCH_TIMEOUT_BASE_MS + (extraBlocks * INIT_FETCH_TIMEOUT_PER_1000_MS),
    INIT_FETCH_TIMEOUT_MAX_MS
  );
}
...
function normalizeRemotePageResponse(data) {
  if (Array.isArray(data)) {
    return { rows: data, hasMore: false, nextPage: null, pageSize: data.length, supportsPaging: false };
  }
  if (Array.isArray(data?.data)) {
    return {
      ...
      pageSize: Number(data.pageSize || data.data.length || 0),
      supportsPaging: data.mode === 'paged' || data.supportsPaging === true || data.nextPage !== undefined
    };
  }
  return { rows: [], hasMore: false, nextPage: null, pageSize: 0, supportsPaging: false };
}

async function fetchRemoteJuntasPaged() {
  const timeoutMs = getAdaptiveInitTimeoutMs();
  const allRows = [];
  let page = 1;

  while (true) {
    const requestUrl = buildApiUrl({ page, limit: REMOTE_PAGE_SIZE });
    debugLog(`Fetching API page=${page} limit=${REMOTE_PAGE_SIZE} timeout=${timeoutMs}ms`);
    ...
    debugLog(`Status page ${page}: ${response.status}`);
    ...
    if (data && data.error === 'network_error') {
      throw new Error("Error de red detectado por Service Worker");
    }

    const pageResult = normalizeRemotePageResponse(data);
    if (page === 1 && !pageResult.rows.length) return pageResult;

    allRows.push(...pageResult.rows);

    if (!pageResult.hasMore || !pageResult.nextPage) {
      return {
        rows: allRows,
        hasMore: false,
        nextPage: null,
        pageSize: pageResult.pageSize,
        supportsPaging: pageResult.supportsPaging
      };
    }
...
function markRemoteSyncSuccess() {
  localStorage.setItem(LAST_REMOTE_SYNC_KEY, String(Date.now()));
}

function shouldAutoFetchRemote() {
  const lastSync = Number(localStorage.getItem(LAST_REMOTE_SYNC_KEY) || 0);
  if (!lastSync) return true;
  return (Date.now() - lastSync) > AUTO_REMOTE_SYNC_TTL_MS;
}
...
async function syncQueue() {
  ...
  const items = await compactSyncQueue();
  if (!items.length) return;
  setSyncStatus('Sincronizando cola...', 'warn');
  let failed = false;
  let processed = 0;
  const chunks = splitIntoChunks(items, SYNC_BATCH_SIZE);
  for (const chunk of chunks) {
    try {
      try {
        await postBatchToSheets(chunk);
      ...
      if (processed % SYNC_QUEUE_PROGRESS_EVERY === 0 || processed === items.length) {
        setSyncStatus(`Sincronizando cola... ${processed}/${items.length}`, 'warn');
...
async function initData(forceRemote = false) {

  debugLog("Iniciando initData...");

  if (!forceRemote && JUNTAS.length > 0 && !shouldAutoFetchRemote()) {
    debugLog("Saltando fetch remoto: cache local vigente");
    setSyncStatus('Datos locales listos', 'ok');
    renderMainList();
    initFilterChips();
    return;
  }
  ...
  try {
    ...
    const remote = await fetchRemoteJuntasPaged();
    const payload = remote.rows;
...
async function postBatchToSheets(events) {
  const normalizedEvents = events.map((ev) => {
    ...
    return {
      ...
      data: {
        ...
        Fecha: data.fecha ?? '',
        Spool: data.spool ?? '',
        'Fecha Spool': data.spoolFecha ?? '',
        'Spool Fecha': data.spoolFecha ?? '',
        spoolFecha: data.spoolFecha ?? '',
        Canista: data.canista ?? '',
        canista: data.canista ?? '',
        'Cañista': data.canista ?? '',
        'Spool Completo': data.spoolCompleto ?? '',
        'Spool clompleto': data.spoolCompleto ?? '',
        'Spool Clompleto': data.spoolCompleto ?? '',
        'Spool completo': data.spoolCompleto ?? '',
        'SPOOL COMPLETO': data.spoolCompleto ?? '',
        spoolCompleto: data.spoolCompleto ?? '',
        Rechazada: data.rechazada ?? false,
        rechazada: data.rechazada ?? false,
        Sch: data.sch ?? '',
        Tipo: data.tipo ?? '',
        Clase: data.clase ?? '',
  ...
  const payload = { action: 'batch', batch: normalizedEvents };
  const json = JSON.stringify(payload);
  ...
  const trySend = async (contentType, body) => {
    ...
    if (parsed && (parsed.ok === false || parsed.success === false || parsed.error)) {
      throw new Error(`API rechazo batch: ${JSON.stringify(parsed)}`);
    }
    if (!parsed && contentTypeResp.includes('text/html')) {
      throw new Error('API devolvio HTML en vez de JSON (revisar deploy/permisos)');
    }
  ...
  try {
    ...
    try {
      ...
      throw new Error(`No se pudo sincronizar lote (plain: ${errPlain.message}; form: ${errForm.message})`);
    }
  }
}
...
async function syncWithSheets(action, item) {
  ...
  try {
    debugLog(`syncWithSheets start action=${action} id=${item?.id || '-'}`);

    if (!navigator.onLine) {
      await enqueueSync(action, item);
      debugLog(`syncWithSheets queued offline action=${action} id=${item?.id || '-'}`);
    ...
    markRemoteSyncSuccess();
    debugLog(`syncWithSheets ok action=${action} id=${item?.id || '-'}`);
    setSyncStatus('Sincronizado...', 'ok');
    ...
    debugLog(`syncWithSheets queued by error action=${action} id=${item?.id || '-'} msg=${err.message}`, true);

    await enqueueSync(action, item);
    setSyncStatus('Guardado local. En cola para sincronizar', 'warn');
    showToast('Guardado local. Se sincroniza cuando haya conexion');
    return { queued: true, synced: false, error: err };
...
async function saveForm() {
  ...
  saveToLocalStorage();
  debugLog(`saveForm prepared id=${record.id} mode=${formMode}`);

  let syncResult = { queued: false, synced: false };
  if (formMode === 'edit' && proposedId !== previousId && currentQRData) {
    await syncWithSheets("delete", { ...currentQRData, id: previousId });
  }
  syncResult = await syncWithSheets("save", record);
  await recordHistory('save', record);
  updateDashboard();
